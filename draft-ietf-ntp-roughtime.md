...
A Roughtime server guarantees that for a query received at t<sub>1</sub>, 
a response sent at t<sub>3</sub> always includes a timestamp created 
at a time t<sub>2</sub> which is between t<sub>1</sub> and t<sub>3</sub>. 
If t<sub>2</sub> does not seem to be within that interval when compared
to information of other servers previous or later, a server inconsistency 
may be detected and reported (and potentially used to impeach the server). 

For details on the reporting process, see {{roughtime-clients}}. For the
reporting to result in impeachment, an additional ecosystem is required
that provides a review an impeachment process. Defining such an ecosystem
is out-of-scope of this document. A simple option could be an online forum
where a court of human observers judge cases after reviewing input reports.
